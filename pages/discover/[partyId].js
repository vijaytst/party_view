...
export default function Discover({ party }) {
          ...
          >
            <div className={'rounded-lg relative discover__card'}>
              ...
              <img
                ...
                src={`https://image.tmdb.org/t/p/original${party.backdrop_path}`}
                alt='event'
              />
            </div>
            <div>
              <div className='flex flex-col mb-8 h-full'>
                <div>
                  ...
                  <div className={'my-3'}>
                    ...
                    <div className={'flex items-center'}>
                      <p className={'font-mono mr-4'}>Length:</p>
                      <div
                        className={'text-gray-900 px-3 mr-2'}
                        style={{ backgroundColor: '#66fcf1' }}
                      >
                        {party.runtime} minutes
                      </div>
                    </div>
                    <div className={'flex items-start'}>
                      <p className={'font-mono mr-4'}>Tagline:</p>
                      <div className={'px-3 mr-2'}>{party.tagline}</div>
                    </div>
                  </div>
                </div>
                <div className='flex items-center'>
...
export async function getServerSideProps(context) {
  const party = await axios.get(
    `https://api.themoviedb.org/3/movie/${context.query.partyId}`,
    {
      params: {
        api_key: API_KEY,
        language: 'en-US',
